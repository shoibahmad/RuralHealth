...
export function VoiceEntryBanner({
    ...
    const defaultDesc =
        language === "en"
            ? "Tap microphone to auto-fill fields in real-time."
            : "रीयल-टाइम में फ़ील्ड भरने के लिए माइक्रोफ़ोन टैप करें।";
    ...
    if (!isSupported) {
        return (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-amber-200 text-sm mb-6">
                Voice entry is not supported in this browser. Please use Chrome or Edge for the best
                experience.
            </div>
        );
    }
        ...
        >
                ...
                <div className="flex items-center gap-4">
                    <div
                        className={cn(
                            ...
                            isListening
                                ? "bg-red-500/20 text-red-400 animate-pulse border border-red-500/30 scale-110"
                                : isProcessing
                                  ? "bg-blue-500/20 text-blue-400 animate-spin border border-blue-500/30"
                        ...
                        <p className="text-sm text-slate-400">
                            {isListening
                                ? language === "en"
                                    ...
                                    : "साफ बोलें (जैसे, 'वजन 70, बीपी 120 ओवर 80')"
                                : isProcessing
                                  ? language === "en"
                                      ? "Gemini is extracting data..."
                                      : "Gemini डेटा निकाल रहा है..."
                ...
                <Button
                    ...
                    className={cn(
                        isListening
                            ...
                            : "bg-teal-500/20 text-teal-300 hover:bg-teal-500/30 border-0",
                    )}
                >
                    {isListening
                        ? language === "en"
            ...
            {(transcript || isListening) && (
                <div className="bg-black/20 rounded-lg p-3 border border-white/5 min-h-[40px]">
                    ...
                    >
                        {transcript ||
                            (language === "en" ? "Waiting for speech..." : "आवाज की प्रतीक्षा...")}
                        {isListening && (
                            <span className="inline-block w-1 h-4 ml-1 bg-teal-400 animate-pulse" />
                        )}
                    </p>
                </div>
            )}
