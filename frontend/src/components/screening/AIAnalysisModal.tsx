            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
                    ...
                    >
                        ...
                        <div
                            className={cn(
                                "absolute -top-24 -right-24 h-64 w-64 blur-[100px] rounded-full opacity-20 bg-gradient-to-br",
                                risk.gradient,
                            )}
                        />
                        ...
                        <div className="relative z-10 flex-1 overflow-y-auto p-8 pt-4 space-y-6 custom-scrollbar">
                            ...
                            <div className="grid md:grid-cols-5 gap-6">
                                ...
                                <motion.div
                                    ...
                                    className={cn(
                                        "md:col-span-2 p-6 rounded-[1.5rem] border relative overflow-hidden flex flex-col justify-between group transition-all hover:shadow-lg",
                                        risk.bg,
                                        risk.border,
                                    )}
                                >
                                ...
                                <motion.div
                                    variants={itemVariants}
                                    className="md:col-span-3 p-6 rounded-[1.5rem] bg-white/[0.03] border border-white/5 flex flex-col group hover:bg-white/[0.05] transition-all"
                                >
                                    ...
                                    <p className="text-slate-200 text-sm leading-relaxed font-medium">
                                        {localLanguage === "hi"
                                            ? analysis.summary_hi || analysis.summary
                                            : analysis.summary}
                                    </p>
                                </motion.div>
                            </div>
                            ...
                            {analysis.formatted_insights && (
                                <motion.div
                                    variants={itemVariants}
                                    className="p-1 rounded-[1.5rem] bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-transparent border border-white/5"
                                >
                                    <div className="bg-slate-900/40 backdrop-blur-xl p-6 rounded-[1.4rem]">
                                        ...
                                        <div className="prose prose-invert prose-sm max-w-none text-slate-300">
                                            <div className="grid gap-3 opacity-90 leading-relaxed italic border-l-2 border-purple-500/30 pl-4 py-1">
                                                {(localLanguage === "hi"
                                                    ? analysis.formatted_insights_hi ||
                                                      analysis.formatted_insights
                                                    : analysis.formatted_insights
                                                )
                                                    .split("\n")
                                                    .map((line: string, i: number) => (
                                                        <p
                                                            ...
                                                            className={cn(
                                                                line.startsWith("**")
                                                                    ? "font-bold text-white not-italic mt-2"
                                                                    : "",
                                                            )}
                                                        >
                                                            {line.replace(/\*\*/g, "")}
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <motion.div variants={itemVariants} className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        {(localLanguage === "hi"
                                            ? analysis.concerns_hi || analysis.concerns
                                            : analysis.concerns
                                        )?.map((item: string, i: number) => (
                                            <div
                                                key={i}
                                                className="group p-4 rounded-2xl bg-red-500/[0.03] border border-red-500/10 flex items-start gap-3 transition-all hover:bg-red-500/[0.06]"
                                            >
                                                <div className="mt-1 h-1.5 w-1.5 rounded-full bg-red-500 shrink-0 shadow-[0_0_8px_rgba(239,68,68,0.5)]" />
                                                <span className="text-xs font-bold text-red-200/80 leading-snug">
                                                    {item}
                                                </span>
                                            </div>
                                        ))}
                                ...
                                <motion.div variants={itemVariants} className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        {(localLanguage === "hi"
                                            ? analysis.recommendations_hi ||
                                              analysis.recommendations
                                            : analysis.recommendations
                                        )?.map((item: string, i: number) => (
                                            <div
                                                key={i}
                                                className="group p-4 rounded-2xl bg-teal-500/[0.03] border border-teal-500/10 flex items-start gap-3 transition-all hover:bg-teal-500/[0.06]"
                                            >
                                                <ArrowRight className="mt-0.5 h-3.5 w-3.5 text-teal-400 shrink-0 group-hover:translate-x-1 transition-transform" />
                                                <span className="text-xs font-bold text-teal-100/80 leading-snug">
                                                    {item}
                                                </span>
                                            </div>
                                        ))}
